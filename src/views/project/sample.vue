<template>
	<div>
		test{{$t('hello')}}
		<el-table :data="dataInGrid"  >
			<el-table-column :label="$t('Sample#')" width="180">
				<template scope="scope">
					<span style="margin-left: 10px">{{ scope.row.dataName }}</span>
				</template>
			</el-table-column>
			<el-table-column :label="$t('Sample Name')" width="180">
				<template scope="scope">{{ scope.row.New }}</template>
			</el-table-column>
			...
			<el-table-column :label="$t('Shipping')">
				<template scope="scope">
...
import config from '@/config'
	export default{
		name:config.prefix+'sample-receive',
		data () {
			return{
				loading:false,
				pagesize: 100,
		...
		methods: {
			...
			getSamplesByOrderId: function(id){
				this.loading=true;
				setTimeout(()=>{this.tableData=[{dataName:'a',New:'123',Processing:'1225',Inqueue:'255'}];
					this.loading=false;
			},2000)
			},
